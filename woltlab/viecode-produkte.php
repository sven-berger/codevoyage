...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        $sql = "INSERT INTO `viecode-rechnung` (pluginname, url, preis) VALUES (:pluginname`, :url, :preis)";
...
$alles_ausgeben = "SELECT * FROM `viecode-rechnung`";
$statement = $connection->query($alles_ausgeben);
$ausgabe = $statement->fetchAll(PDO::FETCH_ASSOC);

// Extras abrufen
$extras_ausgeben = "SELECT * FROM `viecode-rechnung` WHERE (`pluginname` IN ('Installations-/Upgradeservice', 'BrandingFree Lizenz'))";
$statement = $connection->query($extras_ausgeben);
$extra_ausgabe = $statement->fetchAll(PDO::FETCH_ASSOC);

// Statistik ausgeben
$gezielt_ausgeben = "SELECT COUNT(*) AS produkt, SUM(preis) AS gesamtwert FROM `viecode-rechnung`";
...
$wertvollste_produkte = "SELECT pluginname, preis FROM `viecode-rechnung` ORDER BY preis DESC LIMIT 5";
